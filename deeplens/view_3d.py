...
class CrossPoly:
    def __init__(self):
        pass
    def get_poly_data(self) -> PolyData: # type: ignore
        pass
...
class Curve(LineMesh):
    def __init__(self, vertices: np.ndarray, is_loop: bool = True):
...
def group_linemesh(meshes: List[PolyData]) -> PolyData | None:
    """
    ...
    combined_vertices = np.vstack([mesh.points for mesh in meshes]) # type: ignore
...
# ====================================================
# Polygon generation & saving
# ====================================================

def save_lens_3D(save_dir: str,
                 lens: GeoLens,
                 fovs: List[float] = [0.],
...
def geolens_poly(lens: GeoLens,
                 mesh_rings: int = 32,
                 mesh_arms: int = 128,
                 ) -> Tuple[List[CrossPoly], List[CrossPoly], CrossPoly, List[CrossPoly]]:
    """
    ...
    # surf_poly = [PolyData.default() for _ in range(n_surf)]
    bridge_idx = []
    bridge_poly = []
    ap_poly = []
    sensor_poly = PolyData.default()
    ...
    return surf_poly, bridge_poly, sensor_poly, ap_poly # ignore type error
...
def sample_parallel_3D(lens: GeoLens,
    ...
    return Ray(o, d, wvln, device=lens.device) # HANDLE THE DEVICE TYPE MORE ELEGANTLY
...
def curve_from_trace(lens: GeoLens, ray: Ray, delete_vignetting=True):
    ...
    ray_o_records = torch.stack(ray_o_records, dim=0) # type: ignore
...
#     # NEED TO FIX THIS SINCE DUMMY CLASS DOES NOT HAVE "colors" ATTRIBUTE
